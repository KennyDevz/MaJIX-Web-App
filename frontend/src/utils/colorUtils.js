// 1. The Standard 140 HTML Color Names (You don't need to edit this)
const cssColors = {
  "black": "#000000", "silver": "#c0c0c0", "gray": "#808080", "white": "#ffffff", "maroon": "#800000",
  "red": "#ff0000", "fuchsia": "#ff00ff", "green": "#008000", "lime": "#00ff00",
...
// 2. Helper to convert Hex to RGB numbers
const hexToRgb = (hex) => {
  const bigint = parseInt(hex.slice(1), 16);
  return { r: (bigint >> 16) & 255, g: (bigint >> 8) & 255, b: bigint & 255 };
};

// 3. The Logic: Find the closest color name
export const getColorName = (hex) => {
  if (!hex) return "";
  
  // Check for exact match first
  const exactMatch = Object.keys(cssColors).find(key => cssColors[key].toLowerCase() === hex.toLowerCase());
  if (exactMatch) return exactMatch.charAt(0).toUpperCase() + exactMatch.slice(1);

  // If no exact match, find the closest one (Nearest Neighbor)
